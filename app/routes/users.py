...
async def create_user(user: UserForm):
    ...
    if usr:
        email_service.send_email_with_template(
            usr['email'],
            1,
            {'name': usr['full_name'], 'role': usr['role']}        
        )
...
@user.delete(
    ...
    tags=['Users'],
    status_code=204,
    description='En caso de ser admin el borrado será total; en caso contrario será soft delete'
)
...
def retrive_me(
    ...
    data = api_provider.retrieve_data(current_user['role'])
    current_user.update({'data': data})
